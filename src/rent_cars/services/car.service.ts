...
export class CarsLiteService {
  ...
  // src/cars_lite/cars-lite.service.ts
  async findOneWithLikes(id: number, currentUser?: UserSub): Promise<CarIternalLite> {
    const qb = this.carRepo
      .createQueryBuilder('car')
      .leftJoinAndSelect('car.brand', 'brand')
      .leftJoinAndSelect('car.createdBy', 'author')
      .leftJoinAndSelect('car.model', 'model')
      .leftJoinAndSelect('car.engineType', 'engine')
      .leftJoinAndSelect('car.gearBox', 'gear')
      .leftJoinAndSelect('car.city', 'city')
    ...
    // маппинг временного счётчика → isLiked
    if ((car as any)._isLikedTmp != null) {
      ; (car as any).isLiked = !!(car as any)._isLikedTmp
      delete (car as any)._isLikedTmp
    } else {
      ; (car as any).isLiked = false
    }
